  ...
  return (
    ...
    >
      ...
      <div className="xl:col-span-3 h-full flex flex-col gap-6">
        <div className="bg-white rounded-3xl p-8 flex-1 border border-slate-200 shadow-sm overflow-y-auto custom-scrollbar flex flex-col gap-6">
          {chatMessages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center gap-6 opacity-40">
              <div className="w-24 h-24 bg-slate-100 rounded-3xl rotate-12 flex items-center justify-center text-slate-400">
                <MessageSquare size={48} className="-rotate-12 text-emerald-600" />
              </div>
              <div>
                <h2 className="text-2xl font-black text-slate-800">Bank CRM Gemini RAG</h2>
                <p className="max-w-xs mx-auto text-slate-500">Trained on Bangladesh Bank circulars and credit manuals.</p>
              </div>
            </div>
          )}
        ...
        <form onSubmit={handleChat} className="relative">
          <input
            ...
            className="w-full bg-white border border-slate-200 rounded-2xl px-8 py-6 outline-none focus:ring-4 focus:ring-emerald-500/10 shadow-lg text-slate-800 placeholder-slate-400 transition-all"
          />
          <button className="absolute right-4 top-4 bottom-4 aspect-square bg-emerald-600 rounded-xl flex items-center justify-center shadow-lg hover:bg-emerald-700 text-white transition-all hover:scale-105 active:scale-95">
        ...
        <div className="bg-white border border-slate-200 p-6 rounded-3xl shadow-sm">
          ...
          <div className="space-y-4">
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500/10 text-slate-800 placeholder-slate-400 transition-all"
            />
            <div
              className="border-2 border-dashed border-slate-200 rounded-2xl p-6 text-center hover:border-emerald-500/50 transition-colors cursor-pointer group"
              onClick={() => fileInputRef.current?.click()}
            >
              <UploadCloud size={32} className="mx-auto mb-2 text-slate-400 group-hover:text-emerald-600 transition-colors" />
              <p className="text-xs text-slate-500">Click to upload BB Circular (PDF)</p>
              <input type="file" ref={fileInputRef} className="hidden" accept=".pdf" />
            </div>
            <button
              onClick={handleIngest}
              disabled={isIngesting}
              className="w-full bg-emerald-50 text-emerald-600 font-bold py-3 rounded-xl hover:bg-emerald-600 hover:text-white transition-all disabled:opacity-50"
            >
          ...
          <div className="bg-white border border-slate-200 p-6 rounded-3xl shadow-sm">
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar">
              {knowledgeDocs.map((doc) => (
                <div key={doc.id} className="flex items-center justify-between bg-slate-50 rounded-xl px-3 py-2 group border border-slate-100">
